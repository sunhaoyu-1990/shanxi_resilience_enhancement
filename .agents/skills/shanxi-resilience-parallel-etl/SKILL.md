...
执行本 Skill 前，**必须**先阅读 `docs/数据表说明.md`，重点关注：
- 目标表的 UNIQUE 约束（ON CONFLICT 依赖此约束）
- 目标表的字段类型和默认值
- `version_yyyymm` 字段的命名约定（注意大小写：数据库列为 `version_yyyomm`，Python 参数可用 `version_yyyyMM`）
- source_flag 枚举值
- 索引设计（影响并行写入性能）

**禁止**凭记忆假设目标表的约束或字段类型，必须以 `docs/数据表说明.md` 中的数据字典为准。
