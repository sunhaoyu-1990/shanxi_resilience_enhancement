...
def _dedup_by_latest_version(rows: list[dict]) -> list[dict]:
    ...
    for row in rows:
        key = (row["enid"], row["exid"], row["numpath"])
...
def _query_daily_flow_by_vtype(
    ...
    for day_str, table_name in daily_tables:
        ...
        vtype_list = ['1', '2', '3', '4', '11', '12', '13', '14', '15', '16']
        ...
        for sec_id, path_ids in section_to_path_ids.items():
            ...
            # path_id_list = [path_id for (path_id, vtype) in vtype_flow]
            # path_id_list = set(path_id_list)
            # for path_id in path_id_list:
            #     for vt in vtype_list:
            #         flow_results[(path_id, vt)] += vtype_flow.get((path_id, vt), 0)
            for (path_id, vtype), flow in vtype_flow.items():
                flow_results[(path_id, vtype)] += flow

    return flow_results, set(vtype_list)
...
class AffectedOdService(LoggerMixin):
    ...
    def run(self, params: AffectedOdQueryParams, output_path: Optional[str] = None) -> AffectedOdQueryResult:
        ...
        try:
            ...
            # Step 10: 受影响path流量过滤（先按OD对聚合，再过滤）
            # if params.minAffectedPathFlow > 0:
            #     # 对 is_affected=True 的记录按 OD 对聚合流量（跨 path + vehicle_type）
            #     affected_od_flow: dict[tuple[str, str], int] = defaultdict(int)
            #     for r in raw_records:
            #         if r.is_affected:
            #             if r.construction_flow is not None:
            #                 affected_od_flow[(r.enid, r.exid)] += r.construction_flow
            #             if r.same_period_2025_flow is not None:
            #                 affected_od_flow[(r.enid, r.exid)] += r.same_period_2025_flow

            #     # 聚合流量 <= 阈值的 OD 对，其下所有 is_affected=True 记录全部去掉
            #     low_flow_ods = {od for od, total in affected_od_flow.items() if total <= params.minAffectedPathFlow}
            #     before_count = len(raw_records)
            #     raw_records = [
            #         r for r in raw_records
            #         if not r.is_affected or (r.enid, r.exid) not in low_flow_ods
            #     ]
            #     logger.info(
            #         f"受影响path流量过滤: minAffectedPathFlow={params.minAffectedPathFlow}, "
            #         f"OD对聚合后低流量 {len(low_flow_ods)} 个, "
