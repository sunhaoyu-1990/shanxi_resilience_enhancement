...
def parse_args() -> argparse.Namespace:
  ...
  parser = argparse.ArgumentParser(
    description="M1 通行能力评估 - 评估施工方案下路段通行能力",
  )
...
def main() -> int:
  ...
  try:
    ...
    if result.status == TaskStatus.SUCCESS:
      logger.info("=" * 60)
      logger.info(f"M1 通行能力评估任务成功完成")
      logger.info(f"处理记录数: {result.recordsProcessed}")
      ...
      logger.error("=" * 60)
      logger.error(f"M1 通行能力评估任务执行失败")
      logger.error(f"错误信息: {result.errors}")
      logger.error("=" * 60)
      return 1

  except Exception as e:
    logger.exception(f"M1 任务执行异常: {e}")
    return 1
