...
def parse_args() -> argparse.Namespace:
  ...
  parser = argparse.ArgumentParser(
    description="M2 流量与OD迁移统计补全 - 完成流量统计及OD迁移跟踪",
  )
...
def main() -> int:
  ...
  logger.info("=" * 60)
  logger.info(f"M2 流量与OD迁移统计补全任务已启动")
  ...
  try:
    ...
    if result.status == TaskStatus.SUCCESS:
      logger.info("=" * 60)
      logger.info(f"M2 流量与OD迁移统计补全任务成功完成")
      ...
      logger.error("=" * 60)
      logger.error(f"M2 流量与OD迁移统计补全任务执行失败")
      logger.error(f"错误信息: {result.errors}")
      logger.error("=" * 60)
      return 1

  except Exception as e:
    logger.exception(f"M2 任务执行异常: {e}")
    return 1
