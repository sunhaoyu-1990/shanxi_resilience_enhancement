...
用法：
    python -m src.jobs.run_m4 --scheme-id SCH_001 --start-date 2026-04-01 --end-date 2026-04-30
"""
...
def main() -> int:
  ...
  args = parse_args()
  batch_no = generate_batch_no(prefix="M4")

  logger.info("=" * 60)
  logger.info(f"M4 分流路径优化任务已启动")
  ...
  try:
    ...
    if result.status == TaskStatus.SUCCESS:
      logger.info("=" * 60)
      logger.info(f"M4 任务执行成功")
      logger.info(f"已处理记录数：{result.recordsProcessed}")
      logger.info("=" * 60)
      return 0
    else:
      logger.error(f"M4 任务执行失败：{result.status}")
      return 1

  except Exception as e:
    logger.exception(f"M4 任务执行失败：{e}")
    return 1
