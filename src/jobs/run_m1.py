...
用法：
    python -m src.jobs.run_m1 --scheme-id SCH_001 --start-date 2026-04-01 --end-date 2026-04-30
"""
...
def parse_args() -> argparse.Namespace:
  ...
  parser = argparse.ArgumentParser(
    description="M1 通行能力评估 - 计算施工期间路段通行能力",
  )
...
def main() -> int:
  ...
  args = parse_args()
  batch_no = generate_batch_no(prefix="M1")

  logger.info("=" * 60)
  logger.info(f"M1 通行能力评估任务已启动")
  ...
  try:
    ...
    if result.status == TaskStatus.SUCCESS:
      logger.info("=" * 60)
      logger.info(f"M1 通行能力评估任务执行成功")
      logger.info(f"已处理记录数：{result.recordsProcessed}")
      logger.info("=" * 60)
      return 0
    else:
      logger.error(f"M1 任务执行失败：{result.status}")
      return 1

  except Exception as e:
    logger.exception(f"M1 任务执行失败：{e}")
    return 1
