...
用法：
    python -m src.jobs.run_m5 --scheme-id SCH_001 --start-date 2026-04-01 --end-date 2026-04-30
"""
...
def main() -> int:
  ...
  try:
    ...
    if result.status == TaskStatus.SUCCESS:
      logger.info("=" * 60)
      logger.info(f"M5 任务执行成功")
      logger.info(f"已处理记录数：{result.recordsProcessed}")
      logger.info("=" * 60)
      return 0
    else:
      logger.error(f"M5 任务执行失败：{result.status}")
      return 1

  except Exception as e:
    logger.exception(f"M5 任务执行失败：{e}")
    return 1
