...
在开始工作前，请先阅读 `.claude/memory/MEMORY.md` 了解项目状态、上下文和当前进展。
...
### 项目特定规则
在执行任务时，优先使用本项目 `.claude/` 目录下的配置：
- `.claude/rules/` - 项目特定编码规范
- `.claude/memory/` - 项目状态与上下文

### 关键文档（优先查阅）
- `docs/数据表说明.md` - **所有表的权威说明**，含连接信息、数据字典、查询示例，新建表后必须同步更新
- `docs/业务逻辑记录.md` - 业务逻辑条目（BL-00x）
- `.claude/memory/MEMORY.md` - 项目状态、已建立的开发模式、规则表汇总
