...
def test_scheme_b_ksp_optimized(start_node, end_node, version, k=5, max_depth=50):
...
def test_scheme_b_bounded(start_node, end_node, version, max_depth=30, max_paths=100):
    """测试方案B：边界全路径搜索（递归CTE）"""
    print("\n" + "=" * 80)
...
def main():
    ...
    scheme_b2_ok, scheme_b2_time, scheme_b2_count = test_scheme_b_ksp_optimized(
        start_node, end_node, version, k=5, max_depth=50
    )

    # 测试方案B（边界全路径）
    scheme_b_ok, scheme_b_time, scheme_b_count = test_scheme_b_bounded(
        start_node, end_node, version, max_depth=30, max_paths=100
    )
  ...
  SELECT * FROM find_k_shortest_paths_optimized(
      '起点节点', '终点节点', '202512', 5, 50
  );

  -- 方案B: 边界全路径
  SELECT * FROM find_all_paths_bounded(
      '起点节点', '终点节点', '202512', 30, 100
  );
""")
